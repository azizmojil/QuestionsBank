...
- Base templates extend `templates/base.html`; load CSS/JS through `{% static %}` and the `extra_css`/`extra_js` blocks—no inline CSS/JS in templates.
...
- Frontend behavior is plain JavaScript (no frameworks) with `fetch` + explicit `X-CSRFToken` and DOM updates; mirror the patterns in `static/js/base.js` and `static/assessment_runs/js/assessment.js`.
