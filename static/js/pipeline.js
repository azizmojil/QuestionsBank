(function () {
  ...
  const svg = document.getElementById('pipelineLines');

  if (!wrap || !grid || !svg) return;

  // DAG per your confirmed pipeline:
  // version -> self -> routing -> business -> approval
  // version -> lang -> translation -> approval
  // translation -> qbank (third branch)
  const DAG = {
    version: { dependsOn: [] },
  ...
  const STATE = window.PIPELINE_STATE || {};
  ...
  // ===== SVG connector drawing (branching horizontal) =====
  function setSvgSize() {
  ...
  function drawAllLines() {
    // Hide on small screens where grid scrolls
    ...
    // third-level branch from translation
